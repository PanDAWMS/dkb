...
for item in data:
    ...
    if CDSID != '':
        triplethasCDSID = "{docSubject} <{ontology}#hasCDS_ID>"\
            " '{CDS_ID}' .\n".format(**attrsSupportDoc)
        outfile.write(triplethasCDSID)
    if CDSInternal != '':
        triplethasCDSInternal = "{docSubject} <{ontology}#hasCDSInternal>"\
            " '{CDSInternal}' .\n".format(**attrsSupportDoc)
        outfile.write(triplethasCDSInternal)
    if PublicationYear != '':
        triplethasPublicationYear = "{docSubject}"\
            " <{ontology}#hasPublicationYear> {publicationYear} .\n".format(
                **attrsSupportDoc)
        outfile.write(triplethasPublicationYear)
    if URL_Fulltext != '':
        triplethasURLFulltext = "{docSubject} <{ontology}#hasURLFulltext>"\
            " '{URL_Fulltext}' .\n".format(**attrsSupportDoc)
        outfile.write(triplethasURLFulltext)
    if Abstract != '':
        triplethasAbstract = "{docSubject} <{ontology}#hasAbstract>"\
            " '{Abstract}' .\n".format(**attrsSupportDoc)
    ...
    if "keywords" in item:
        ...
        for keyword in Keywords:
            ...
            tripletKeyword = "{docSubject} <{ontology}#hasKeyword>"\
                " '{Keyword}' .\n".format(**attrKeyword)
    ...
    if "authors" in item:
        ...
        for author in authors:
            ...
            if full_name != '':
                triplethasFullName = "{authorSubject}"\
                    " <{ontology}#hasFullName> '{full_name}' .\n".format(
                        **attrrsAuthor)
                outfile.write(triplethasFullName)
            if email != '':
                triplethasEmail = "{authorSubject} <{ontology}#hasEmail>"\
                    " '{email}' .\n".format(**attrrsAuthor)
                outfile.write(triplethasEmail)
            if affiliation != '':
                triplethasAffiliation = "{authorSubject}"\
                    " <{ontology}#hasAffiliation> '{affiliation}' .\n".format(
                        **attrrsAuthor)
                outfile.write(triplethasAffiliation)
            hasAuthorTriplet = "{docSubject} <{ontology}#hasAuthor>"\
                " {authorSubject} .\n".format(**attrrsAuthor)
