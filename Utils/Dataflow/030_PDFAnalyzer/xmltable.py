...
#        print "\nTABLE WITH HEADER", header
        ...
        while r > 0:
            if not max_diff:
                max_diff = row_centery(self.rows[r]) - row_centery(self.rows[r-1])
            else:
                diff = row_centery(self.rows[r]) - row_centery(self.rows[r-1])
#                print "DIFF BETWEEN", self.row_text(num = r), "AND", self.row_text(num = r - 1), ":", diff
...
#                    print "LINES WITH TEXT", line.text, "AND", row[i].text, "OVERLAP - MERGING"
