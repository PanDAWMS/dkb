...
def main(argv):
    ...
    stage.add_argument('--usercert', help='PEM certificate file', required=True)
...
def init_ami_client(userkey, usercert):
    ...
    global ami_client
    try:
        ami_client = pyAMI.client.Client('atlas', key_file=userkey, cert_file=usercert)
        AtlasAPI.init()
    except Exception:
        sys.stderr.write(
            "Could not establish pyAMI session. Are you sure you have a valid certificate?")
        sys.exit(1)

def process(stage, message):

    stage.output(pyDKB.dataflow.messages.JSONMessage(amiPhysValues(message.content())))
