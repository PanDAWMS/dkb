...
Definition of an abstract class for Dataflow Data Processing Stages.
    ...
    OPTIONS:
     ...
     -i, --input-dir  DIR     - base directory for relative input file names
                                (for local and HDFS sources)

     -d, --dest       {f|s|h} - where to send data to: local (f)iles, (s)tdin, (h)dfs
     ...
     -m, --mode       MODE     - MODE:
                                 (f)ile      = --source f --dest f
                                               (dest can be rewritten with 's' or 'h')
                                 (s)tream    = --source s --dest s
                                               (source can be rewritten with 'h')
                                 (m)apreduce = --source s --dest s
                                               (source can be rewritten with 'h')
...
class AbstractProcessorStage(AbstractStage):
  ...
  * Iterable object for input data
  ...
  def __init__(self,description="DKB Dataflow data processing stage."):
    ...
    * set description
    * define common Processor arguments (input, --hdfs, --hdfs-dir, --output, ...)
    * ...
    """
  ...
  def defaultArguments(self):
    ...
    self.add_argument('-s', '--source', action='store', type=str, nargs='?',
                       help=u'Where to get data from: local (f)iles, (s)tdin, (h)dfs (same as --hdfs).',
                       default='f',
                       const='f',
                       choices=['f', 's', 'h'],
                       dest='source'
                       )
    self.add_argument('-i', '--input-dir', action='store', type=str, nargs='?',
                       help=u'Base directory in local file system or in HDFS (for relative FILE names)',
                       default='',
                       const='',
                       metavar='DIR',
                       dest='input_dir'
    ...
    self.add_argument('-d', '--dest', action='store', type=str, nargs='?',
                       help=u'Where to send results: local (f)iles, (s)tdout, (h)dfs (same as --hdfs).',
                       default='f',
                       const='f',
                       choices=['f', 's', 'h'],
                       dest='dest'
                       )
    self.add_argument('-o', '--output-dir', action='store', type=str, nargs='?',
                       help=u'Directory or file for output files (local or HDFS). '
                             'If the directory doesn\'t exist, use "dir/" instead of "dir".',
                       default='',
                       const='output/',
                       metavar='DIR',
                       dest='output_dir'
    ...
    self.add_argument('--hdfs', action='store', type=bool, nargs='?',
                       help=u'Source files are stored in HDFS; if no input FILE specified, '
                             'filenames will come to stdin. '
                             'This option is equivalent to "--source h --dest h"',
                       default=False,
                       const=True,
                       metavar='HDFS',
                       dest='hdfs'
  ...
  def __hdfsFiles(self):
    filenames = self.ARGS.input_files
    if not filenames: filenames = iter(sys.stdin.readline)
    ...
    for f in filenames:
      ...
      try:
        if os.access(name, os.F_OK):
          os.remove(name)
        subprocess.check_call(cmd, shell=True, stderr=DEVNULL, stdout=DEVNULL)
