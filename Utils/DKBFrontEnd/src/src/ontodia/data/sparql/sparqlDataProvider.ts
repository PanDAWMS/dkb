...
export class SparqlDataProvider implements DataProvider {
    ...
    filter(params: FilterParams): Promise<Dictionary<ElementModel>> {
        ...
        WHERE { __GRAPH_LIMIT_BEGIN__
            {
                SELECT DISTINCT ?inst ?score WHERE { __GRAPH_LIMIT_BEGIN__
                    ${elementTypePart}
                    ${refQueryPart}
                    ${textSearchPart}
                ...
                __GRAPH_LIMIT_END__ } ORDER BY DESC(?score) LIMIT ${params.limit} OFFSET ${params.offset}
            }
            ${resolveTemplate(this.settings.filterElementInfoPattern, {dataLabelProperty: this.dataLabelProperty})}
        __GRAPH_LIMIT_END__ } ORDER BY DESC(?score)
        `;

        // TODO: we don't always have 'inst' property in results.  Why?
...
export function executeSparqlQuery<Binding>(endpoint: string, query: string, method: SparqlQueryMethod): Promise<SparqlResponse<Binding>> {
    ...
    return internalQuery.then((response): Promise<SparqlResponse<Binding>> => {
        if (response.ok) {
            ...
            const error = new Error(`${response.statusText}: ${query}`);
